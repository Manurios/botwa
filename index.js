...
prefix = "/"
name = "~ Manurios | B O T"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN ♡"
rmenu = "HAI TEMEN 👋"
limitt = 10
ban = []
userpremium = ["6283141926935@s.whatsapp.net"] //ubah nomer kalian
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const Manurios = ["083141926935@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["6283141926935@s.whatsapp.net"] // owner number ubah aja
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
					...
					if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama/umur\nContoh : ${prefix}daftar Manurios/18/Banjarmasin`)
					reg = `${body.slice(8)}`
					jeneng = reg.split("/")[0];
					umure = reg.split("/")[1];
					asal = reg.split("/")[2];
			...
			if (isBanned) return reply(mess.only.benned)
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
					await costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal), text, Manurios, rmenu)
    				break
